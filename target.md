...
### [ ] Tối ưu hóa hiệu năng Import (Giảm thiểu Exclusive Lock bảng)
* **Vấn đề**: Việc sử dụng lệnh `objects.all().delete()` ở đầu luồng Import Excel cộng với việc file như *Sổ chi tiết* đã phình lên hơn 7.000 dòng khiến tiến trình kéo dài 1-2 phút. Trong suốt quá trình chạy khối `transaction.atomic()`, bảng bị khóa độc quyền (Exclusive Lock), có thể gây treo/timeout các API lấy dữ liệu của Frontend.
* **Đề xuất**: 
  - Nâng cấp cơ chế nạp dữ liệu sang **Swap Table** (ghi vào bảng tạm sau đó thực hiện lệnh đổi tên bảng `RENAME TABLE` chỉ mất vài mili-giây).
  - Hoặc chuyển sang cơ chế xóa theo kỳ kế toán cụ thể thay vì Wipe and Reload toàn bộ bảng.
* **Nơi xử lý**: Hàm `auto_import_excel_from_folder` trong [tasks.py](file:///d:/Sources/dashboard-report/accounting/tasks.py).
...
* **Nơi xử lý**: Cấu trúc bảng `BUPerformance` trong [models.py](file:///d:/Sources/dashboard-report/accounting/models.py) và logic tổng hợp trong [tasks.py](file:///d:/Sources/dashboard-report/accounting/tasks.py).
...
* **Nơi xử lý**: [views.py](file:///d:/Sources/dashboard-report/accounting/views.py).

---

## ✅ 4. Các Nhiệm Vụ Đã Hoàn Thành (Completed Tasks)

### [x] Tối ưu hóa logic Import và cập nhật Bot MISA tự động
* **Trạng thái**: Đã hoàn thành (Cập nhật mới nhất).
* **Khắc phục lỗi Import**: Sửa triệt để lỗi `Customer matching query does not exist` do dữ liệu Excel trống hoặc có khoảng trắng mã khách hàng bằng cách can thiệp tự động gán `None` vào `before_import_row`. Đảm bảo luồng nạp hàng nghìn dòng hoạt động an toàn trong `transaction.atomic()` với khả năng Rollback khi hỏng dòng dữ liệu.
* **Nâng cấp Bot MISA Automation**: Bỏ thao tác chọn *Kỳ báo cáo* ở báo cáo *Tuổi nợ KH*; Thêm logic tự động mở dropdown tìm chọn *Bậc 1* và tick chính xác các tài khoản `111, 112, 341` ở báo cáo *Sổ chi tiết*. Đã bổ sung các nhịp ngưng (delay) chờ DOM render giúp script ổn định hơn.
* **Giám sát MISA**: Thêm hệ thống chụp ảnh màn hình lưu file `MISA_Error_*.png` và báo lỗi Traceback ra console nếu gặp sự cố giao diện MISA thay đổi.

### [x] Xây dựng bảng Log Import dữ liệu
* **Trạng thái**: Đã hoàn thành.
* **Vấn đề cũ**: Hệ thống chỉ di chuyển file vào `success/` hoặc in terminal, khó trace lỗi từ xa khi import fail.
* **Cải tiến**: Thiết kế model `ImportLog` lưu trữ trạng thái `SUCCESS`/`ERROR`, thời gian chạy `start_time`, `end_time` và báo lỗi chi tiết đến từng dòng dữ liệu hỏng. Đã đưa bảng này lên giao diện Django Admin cho người quản trị dễ dàng theo dõi toàn bộ lịch sử chạy của Celery Task.
